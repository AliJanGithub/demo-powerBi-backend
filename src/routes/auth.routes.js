...
router.post('/login', loginValidation, validate, async (req, res, next) => {
  try {
    // 🛑 TEMPORARY BYPASS: This block enforces multi-tenancy logic.
    // Since tenantResolver is commented out and you want to bypass 
    // tenant logic for now, we comment this out.
    /*
    if (!req.tenant && !req.hostname.includes('localhost')) {
      return res.status(400).json({
        success: false,
...
router.post(
  ...
  async (req, res, next) => {
    try {
      // 🛑 TEMPORARY BYPASS: Commenting out the multi-tenancy check here as well.
      /*
      if (!req.tenant && !req.hostname.includes('localhost')) {
        return res.status(400).json({
          success: false,
